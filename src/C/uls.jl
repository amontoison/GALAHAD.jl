...
function uls_initialize(solver, data, control, status)
    @ccall libgalahad_all.uls_initialize(solver::Ptr{Cchar}, data::Ptr{Ptr{Cvoid}},
                                         control::Ptr{uls_control_type},
                                         status::Ptr{Cint})::Cvoid
end

function uls_read_specfile(control, specfile)
    @ccall libgalahad_all.uls_read_specfile(control::Ptr{uls_control_type},
                                            specfile::Ptr{Cchar})::Cvoid
end

function uls_factorize_matrix(control, data, status, m, n, type, ne, val, row, col, ptr)
    @ccall libgalahad_all.uls_factorize_matrix(control::Ptr{uls_control_type},
                                               data::Ptr{Ptr{Cvoid}}, status::Ptr{Cint},
                                               m::Cint, n::Cint, type::Ptr{Cchar}, ne::Cint,
                                               val::Ptr{Float64}, row::Ptr{Cint},
                                               col::Ptr{Cint}, ptr::Ptr{Cint})::Cvoid
end

function uls_reset_control(control, data, status)
    @ccall libgalahad_all.uls_reset_control(control::Ptr{uls_control_type},
                                            data::Ptr{Ptr{Cvoid}}, status::Ptr{Cint})::Cvoid
end

function uls_solve_system(data, status, m, n, sol, trans)
    @ccall libgalahad_all.uls_solve_system(data::Ptr{Ptr{Cvoid}}, status::Ptr{Cint},
                                           m::Cint, n::Cint, sol::Ptr{Float64},
                                           trans::Bool)::Cvoid
end

function uls_information(data, inform, status)
    @ccall libgalahad_all.uls_information(data::Ptr{Ptr{Cvoid}},
                                          inform::Ptr{uls_inform_type},
                                          status::Ptr{Cint})::Cvoid
end

function uls_terminate(data, control, inform)
    @ccall libgalahad_all.uls_terminate(data::Ptr{Ptr{Cvoid}},
                                        control::Ptr{uls_control_type},
