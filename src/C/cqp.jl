...
function cqp_initialize(data, control, status)
    @ccall libgalahad_all.cqp_initialize(data::Ptr{Ptr{Cvoid}},
                                         control::Ptr{cqp_control_type},
                                         status::Ptr{Cint})::Cvoid
end

function cqp_read_specfile(control, specfile)
    @ccall libgalahad_all.cqp_read_specfile(control::Ptr{cqp_control_type},
                                            specfile::Ptr{Cchar})::Cvoid
end

function cqp_import(control, data, status, n, m, H_type, H_ne, H_row, H_col, H_ptr, A_type,
                    A_ne, A_row, A_col, A_ptr)
    @ccall libgalahad_all.cqp_import(control::Ptr{cqp_control_type}, data::Ptr{Ptr{Cvoid}},
...
function cqp_reset_control(control, data, status)
    @ccall libgalahad_all.cqp_reset_control(control::Ptr{cqp_control_type},
...
function cqp_solve_qp(data, status, n, m, h_ne, H_val, g, f, a_ne, A_val, c_l, c_u, x_l,
    ...
    @ccall libgalahad_all.cqp_solve_qp(data::Ptr{Ptr{Cvoid}}, status::Ptr{Cint}, n::Cint,
...
function cqp_solve_sldqp(data, status, n, m, w, x0, g, f, a_ne, A_val, c_l, c_u, x_l, x_u,
    ...
    @ccall libgalahad_all.cqp_solve_sldqp(data::Ptr{Ptr{Cvoid}}, status::Ptr{Cint}, n::Cint,
...
function cqp_information(data, inform, status)
    @ccall libgalahad_all.cqp_information(data::Ptr{Ptr{Cvoid}},
                                          inform::Ptr{cqp_inform_type},
                                          status::Ptr{Cint})::Cvoid
end

function cqp_terminate(data, control, inform)
    @ccall libgalahad_all.cqp_terminate(data::Ptr{Ptr{Cvoid}},
                                        control::Ptr{cqp_control_type},
