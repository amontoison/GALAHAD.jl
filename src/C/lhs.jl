...
function lhs_initialize(data, control, inform)
    @ccall libgalahad_all.lhs_initialize(data::Ptr{Ptr{Cvoid}},
                                         control::Ptr{lhs_control_type},
                                         inform::Ptr{lhs_inform_type})::Cvoid
end

function lhs_read_specfile(control, specfile)
    @ccall libgalahad_all.lhs_read_specfile(control::Ptr{lhs_control_type},
                                            specfile::Ptr{Cchar})::Cvoid
end

function lhs_ihs(n_dimen, n_points, seed, X, control, inform, data)
    @ccall libgalahad_all.lhs_ihs(n_dimen::Cint, n_points::Cint, seed::Ptr{Cint},
...
function lhs_get_seed(seed)
    @ccall libgalahad_all.lhs_get_seed(seed::Ptr{Cint})::Cvoid
end

function lhs_terminate(data, control, inform)
    @ccall libgalahad_all.lhs_terminate(data::Ptr{Ptr{Cvoid}},
                                        control::Ptr{lhs_control_type},
                                        inform::Ptr{lhs_inform_type})::Cvoid
end
